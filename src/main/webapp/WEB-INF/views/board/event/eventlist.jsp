	  ...
	  <div class="container">
            ...
            <div class="row multi-columns-row post-columns bg-white">
              ...
              <div class="col-sm-6 col-md-4 col-lg-4">
                <div class="post">    
                 <div>
              <c:set var="content" value="${eboard.eventContent }"/><!--전체콘텐츠-->
              <c:set var="imgfront" value="${fn:substringBefore(content,'<img') }"/><!--앞부분-->
              <c:set var="imgback" value="${fn:substringAfter(content,'></p>') }"/><!--뒷부분-->
              <c:set var="imgfront1" value="${fn:indexOf(content,'<img') }"/><!--앞부분시작수-->
              <c:set var="imgback2" value="${fn:indexOf(content,'></p>')+1}"/><!--뒷부분시작수+1-->
              <c:set var="realimage" value="${fn:substring(content,imgfront1,imgback2) }"/><!-- 이미지만추출-->
      			 ${realimage}
                 </div>
                  <div class="post-header font-alt">
                    <h2 class="post-title">
                    <a class="text-dark" href="/eventview/${eboard.eventCode}">
                    ${eboard.eventTitle }</a></h2>
                    <div class="post-meta">By&nbsp; ${eboard.eventWriter }
                    &nbsp;| <fmt:formatDate value="${eboard.eventRegdate }" pattern="yyyy-MM-dd" /> | 
                    <i class="fa fa-eye" aria-hidden="true"></i> ${eboard.eventHitcount } 
                    </div>
                  </div>
                  <div class="post-more">
                  <a class="more-link text-dark" style="text-align:right" href="/eventview/${eboard.eventCode}">
                  <I>Read more</I></a></div>
                </div>
              </div>
              </c:forEach>
